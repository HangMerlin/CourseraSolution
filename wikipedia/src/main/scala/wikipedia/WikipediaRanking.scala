...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  val conf: SparkConf = new SparkConf(true)
    .setMaster("local[*]")
    .setAppName("WikipediaAssignment")
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap(article => langs.map(lang => lang -> article).filter(t => t._2.mentionsLanguage(t._1))).groupByKey()
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    rdd.flatMap(article => langs.map(lang => (lang , if (article.mentionsLanguage(lang)) 1 else 0))).reduceByKey(_ + _).collect.toList.sortBy(_._2)(Ordering[Int].reverse)
