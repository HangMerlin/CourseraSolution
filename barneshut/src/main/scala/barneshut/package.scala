...
package object barneshut {
  ...
  sealed abstract class Quad extends QuadInterface {
    ...
    def contains(b: Body): Boolean = centerX - size / 2 <= b.x && b.x < centerX + size / 2 && centerY - size / 2 <= b.y && b.y < centerY
  }
  ...
  case class Empty(centerX: Float, centerY: Float, size: Float) extends Quad {
    ...
    def total: Int = 0
    def insert(b: Body): Quad =
//      if ( centerX - size / 2 <= b.x && b.x < centerX + size / 2 && centerY - size / 2 <= b.y && b.y < centerY)
...
//      else this
  ...
  case class Fork(nw: Quad, ne: Quad, sw: Quad, se: Quad) extends Quad {
    ...
    def insert(b: Body): Fork = {
      ...
      else if (ne.contains(b)) Fork(nw, ne.insert(b), sw, se)
      else if (sw.contains(b)) Fork(nw, ne, sw.insert(b), se)
      else if (se.contains(b)) Fork(nw, ne, sw, se.insert(b))
      else this
    }
  }
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: coll.Seq[Body]) extends Quad {
    ...
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        ...
        bodies.foldLeft(fork)((quad, b) => quad.insert(b))
      }
  ...
  class Body(val mass: Float, val x: Float, val y: Float, val xspeed: Float, val yspeed: Float) {
    ...
    def updated(quad: Quad): Body = {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        ...
        case f: Fork =>
          if (f.size / distance(x, y, f.centerX, f.centerY) < delta) addForce(f.mass, f.massX, f.massY)
          else {
            traverse(f.nw)
            traverse(f.ne)
            traverse(f.sw)
